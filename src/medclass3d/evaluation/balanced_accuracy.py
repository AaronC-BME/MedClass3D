...
class BalancedAccuracy(Metric):
    """Balanced accuracy = mean of per-class (sensitivity + specificity) / 2.

    Supports ``multiclass`` and ``multilabel`` tasks. For multilabel, ``preds``
    may be logits (sigmoid is applied) or probabilities; the threshold is
    applied before counting stats.
    """
